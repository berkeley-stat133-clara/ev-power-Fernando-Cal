...
```{r}
str_extract(energy_sources, "MWh|GWh|kWH")
```

c.  Clean the names to keep only the energy source name, trimming whitespace and removing parentheses.

```{r}
clean_sources <- str_remove_all(energy_sources, "\\(.*\\)")
str_trim(clean_sources)
```
...
unit <- c("MWh", "GWh", "kWh")
energy_clean <- tibble(
    source = str_remove_all(energy_sources,"\\(.*\\)") |> str_trim(),
    unit = str_extract(energy_sources, "MWh|GWh|kWH")
)
